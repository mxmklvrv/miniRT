...
int parse_cam(char *line, t_scene *scene)
{
    ...
    scene->cam.fov = fov;
    scene->cam.yaw = atan2f(scene->cam.orient.direction.x, scene->cam.orient.direction.z); // added for test
    scene->cam.pitch = asinf(scene->cam.orient.direction.y);                               // adde for test
    free_array(res);
    return (0);
}
