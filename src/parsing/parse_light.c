...
int	parse_light(char *line, t_scene *scene)
{
	...
	if (scene->qt_light > 1)
		return (error(ERR_LIGT_QTY, NULL), 1);
	if (count_elements(line) != 3) // or 2 for mandotary
		return (error(ERR_LIGT_SPEC, scene->err_m), 1);
	res = ft_split(line, ' ');
	if (!res)
	...
	if (parse_vector(res[0], &scene->light.pos, -100.0f, 100.0f) == 1)
		return (error(ERR_LIGT_POS, scene->err_m), free_array(res),
			1);
	if (parse_float(res[1], 0.0f, 1.0f, &bright) == 1)
		return (error(ERR_LIGT_BRIT, scene->err_m),
			free_array(res), 1);
	if (parse_rgb(res[2], &colour) == 1) // this is for bonus
		return (error(ERR_LIGT_COLOR, scene->err_m), free_array(res),
			1);
