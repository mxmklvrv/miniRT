...
int	parse_ambient(char *line, t_scene *scene)
{
	...
	if (scene->qt_ambiant > 1)
		return (error(ERR_AMB_QTY, scene->err_m), 1);
	if (count_elements(line) != 2)
		return (error(ERR_AMB_SPEC, scene->err_m), 1);
	res = ft_split(line, ' ');
	if (!res)
	...
	if (parse_float(res[0], 0.0f, 1.0f, &ratio) == 1)
		return (error(ERR_AMB_RATI, scene->err_m), free_array(res),
			1);
	if (parse_rgb(res[1], &colour) == 1)
		return (error(ERR_AMB_COLR, scene->err_m), free_array(res),
			1);
