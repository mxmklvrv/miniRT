...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/01 21:43:28 by akolupae          #+#    #+#             */
/*   Updated: 2026/04/01 21:43:29 by akolupae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	lighting(t_scene *scene, t_hit hit, t_ray ray)
{
	...
	color = color_mix_light(hit.shape->color, lighting.ambient);
	lighting.normal.origin = get_ray_point(ray, get_closest_hit(hit));
	lighting.normal.direction = get_normal(hit.shape,
	...
	while (llist)
	{
		if (!is_inside(scene->cam.orient.origin, hit.shape)
			&& !is_inside(llist->light->pos, hit.shape))
			add_current_light(*llist->light, &lighting, scene, ray);
		llist = llist->next;
	}
	color = color_mix_light(hit.shape->color,
	...
	t_scene *scene, t_ray ray)
{
	...
	if (is_in_shadow(scene, light, new_ray(lighting->normal.origin,
	...
	t_vec3 normal_vector, t_ray ray)
{
	...
	if (reflect_angle < 0)
		return (0);
	shininess = powf(reflect_angle, 64.0);
	...
	float *distance)
{
	...
	*distance = vector_magnitude(vector_substract(light.pos, light_ray.origin));
	...
	while (obj_list)
	{
		find_closest_hit(light_ray, obj_list->shape, &closest);
