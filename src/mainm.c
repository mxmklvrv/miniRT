...
int	parse_sphere(char *line, t_scene *scene)
{
	...
	if (parse_vector(res[0], &sphere->sp_center, -100.0f, 100.0f) == 1)
		return (error("Invalid sphere center", scene->err_m), free(sphere), free_array(res),
			1);
	diameter = 0;
	colour = 0;
	if (parse_float(res[1], 0.0f, 100.0f, &diameter) == 1)
		return (error("Invalid sphere diameter", scene->err_m), free(sphere), free_array(res),
			1);
	if (parse_rgb(res[2], &colour) == 1)
		return (error("Invalid sphere colour", scene->err_m), free(sphere), free_array(res),
			1);
	sphere->diameter = diameter;
	sphere->colour = colour;
	if (add_to_list(&scene->obj_list, sphere, SP, sphere->colour) == 1)
		return (error("Failed adding sphere to the list", scene->err_m), free_array(res),
			free(sphere), 1);
	printf("SPHERE\n");
...
int	parse_plane(char *line, t_scene *scene)
{
	...
	if (parse_vector(res[0], &plane->pl_point, -100.0f, 100.0f) == 1)
		return (error("Invalid coordinates of a point in plane", scene->err_m), free(plane),
			free_array(res), 1);
	if (parse_vector(res[1], &plane->normal, 0.0f, 1.0f) == 1)
		return (error("Invalid normal vector for plane", scene->err_m), free(plane),
			free_array(res), 1);
	colour = 0;
	if (parse_rgb(res[2], &colour) == 1)
		return (error("Invalid plane colour", scene->err_m), free(plane), free_array(res), 1);
	plane->colour = colour;
	if (add_to_list(&scene->obj_list, plane, PL, plane->colour) == 1)
		return (error("Failed adding plane to the list", scene->err_m), free(plane),
			free_array(res), 1);
	printf("PLANE\n");
...
int	parse_cylinder(char *line, t_scene *scene)
{
	...
	if (parse_vector(res[0], &cylinder->cy_center, -100.0f, 100.0f) == 1)
		return (error("Invalid coordinates of the center of the cylinder", scene->err_m),
	...
	if (parse_float(res[2], 0.0f, 100.0f, &diameter) == 1)
		return (error("Invalid cylinder diameter", scene->err_m), free(cylinder),
			free_array(res), 1);
	...
	if (add_to_list(&scene->obj_list, cylinder, CY, cylinder->colour) == 1)
		return (error("Failed adding cylinder to the list", scene->err_m), free(cylinder),
			free_array(res), 1);
	printf("CYLINDR\n");
...
int	parse_ambient(char *line, t_scene *scene)
{
	...
	if (parse_float(res[0], 0.0f, 1.0f, &ratio) == 1)
		return (error("Invalid Ambient ratio", scene->err_m), free_array(res), 1);
	if (parse_rgb(res[1], &colour) == 1)
		return (error("Invalid Ambient colour", scene->err_m), free_array(res), 1);
...
int	parse_light(char *line, t_scene *scene)
{
	...
	if (parse_vector(res[0], &scene->light.pos, -100.0f, 100.0f) == 1)
		return (error("Invalid light position", scene->err_m), free_array(res), 1);
	if (parse_float(res[1], 0.0f, 1.0f, &bright) == 1)
		return (error("Invalid light brightness", scene->err_m), free_array(res), 1);
	if (parse_rgb(res[2], &colour) == 1) // this is for bonus
		return (error("Invalid light colour", scene->err_m), free_array(res), 1);
...
int	parse_cam(char *line, t_scene *scene)
{
	...
	if (parse_vector(res[0], &scene->cam.view_point, -100.0f, 100.0f) == 1)
		return (error("Invalid camera view point", scene->err_m), free_array(res), 1);
	if (parse_vector(res[1], &scene->cam.orient, 0.0f, 1.0f) == 1)
		return (error("Invalid camera orientation", scene->err_m), free_array(res), 1);
	fov = 0;
...
int	pars_input_file(char *file, t_scene *scene)
{
	...
	while (line)
	{
		...
		scene->err_m = trimmed; // need to free ?? 
